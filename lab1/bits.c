...
int bitMask(int highbit, int lowbit) {
  return (((~0x0)<<lowbit)&(~(((~0x0)<<highbit)<<1)));
}
/* 
...
int isPower2(int x) {
    ...
    return sign & ( ! ( ~ ( ( ~ 0x1 ) ^ ((sum + (sum >> 8)) & 0x3F)))); 
}
/* 
...
int reverseBytes(int x) {
    return 0x00+((0xFF&x)<<24)+(((0xFF<<8)&x)<<8)+(((0xFF<<16)&x)>>8)+((((0xFF<<24)&x)>>24)&0xFF);
}
/* 
