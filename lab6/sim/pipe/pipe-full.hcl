...
# Name: Jianbang Yang
# Description:
#   1.  Add iaddl and leave instructon, it's almost the same of
#       SEQ processor.
#
#   2.  Add load-forwarding, referring to the solution of Exercise 5.
#
#   3.  Add a new instruction 'cingj'. When Cond_t C_G is YES,  
#       %eax will increase by 1 and jump unconditionally to the
#       specific position. It only work in the pattern below:
#
#          ---------------------------------------------
#          |      rmmovl rA, rB                        |
#          |      INST ( In my case, andl %esi,%esi )  | 
#          |      jle Label                            |
#          |      iaddl $1,%eax                        |
#          |Label:                                     |
#          ---------------------------------------------
#         
#         Because the normal Y86 doesn't have conditonal iaddl, the 
#       processor has to deal with lots of mispredictings which waste
#       many cycles.I made a new instruction in order to implement 
#       such a funtion.
#         I marked rmmovl's Y86 codes with 0x46 instead of 0x40 so it
#       only works in men-written Y86 codes.
#         So normal processors will carry out jle and iaddl maybe, 
#       while my processor will detect the marked rmmovl, read
#       jle as cingj, and then do the works below:
#
#                 -----------------------------------------
#                 | Stage      |   cingj V                |
#                 |------------|--------------------------|
...
#                 | Memory     |                          |
#                 |------------|--------------------------|
#                 | Write Back |  R[REAX]<-Cond?valE:valA |
#                 |------------|--------------------------|
#                 | Update PC  |   PC <- valP             |
#                 -----------------------------------------
#       
#         The final trick is that I use the reserved position of pop2,
#       I change it's original function to simply my implement. Even 
#       without pop2. I can also implement such a function by more
#       complex hcl codes.
#   
#/* $begin pipe-all-hcl */
####################################################################
...
# Instruction code for pop2 instruction
intsig ICINGJ    'I_POP2'
...
bool instr_valid = f_icode in 
	{ INOP, IHALT, IRRMOVL, IIRMOVL, IRMMOVL, IMRMOVL,
	  IOPL, IJXX, ICALL, IRET, IPUSHL, IPOPL, IIADDL, ILEAVE, ICINGJ };
...
bool need_valC =
	f_icode in { IIRMOVL, IRMMOVL, IMRMOVL, IJXX, ICALL, IIADDL, ICINGJ };
