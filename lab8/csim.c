...
/* getinfo_pos - get index info from the adress */
...
/* print_detail - print infomation for each memory access */
...
int main(int argc, char *argv[])
{
    ...
    if(!(trace_file = fopen(file_name,"r"))){
        app_error("No such file or diretory",file_name);
    }
    ...
    while(fscanf(trace_file,"%c %lx,%d",&access_type,&address,&byte) != EOF){
        ...
        if(!(access_type == 'L' || access_type == 'M' || access_type == 'S')){
            continue;
        }

        /* Read infomation from address */
